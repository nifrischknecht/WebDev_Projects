...
// const selectSunday = document.querySelector('.lineupFestival__linupAllDay');
...
const openAccordions = document.querySelector('.infosFestival__allAccordions');


// -------------------- *  ALL LET OUTSIDE FUNCTIONS  * -------------------- \\
...
if (closeMenu) {
    ...
    closeMenu.addEventListener('click', Tools.delegate('.menu-close__cross', function (event) {
        // close menu now
        toggleMenu('close');
    }));
};
...
// LOGIN: open and close the login-box in header
if (login) {
...
// FUNCTION for toggleLogin
function toggleLogin(force_status) {
    const loginArea = document.querySelector('#box-outside');
    loginArea.classList.toggle('box-outside--active', force_status === 'close' ? false : undefined);
    // Toggle Login Button class
    const loginBtn = document.querySelector('.actions__login');
    loginBtn.classList.toggle('actions__login--active', force_status === 'close' ? false : undefined);
    // Toggle Menu login button class Always toggle both menu buttons so they are in sync so we can use the same toggleLogin() method for both buttons
...
// MENU-LOGIN: open and close the login-box in menu-heder
if (loginMenu) {
...
// FILTER-LOGIN: open and close the login-box in filter-header
if (loginFilter) {
...
if (filterBtn) {
    ...
    filterBtn.addEventListener('click',Tools.delegate ('.filterBtn button', function(event) {
...
// FILTER: close (only for Smartphone)
if (closeFilterBtn) {

    closeFilterBtn.addEventListener('click',Tools.delegate ('.menuFilter__hamburger', function(event) {
...
// MAGAZINE: Aricles for Mobile-Page: read and close articles
if (artOpen) {

    artOpen.addEventListener('click',Tools.delegate ('.magazineS__readBtn', function(event) {
...
// DISCOVERIES: Audios-Genres -> play and stop
if (audio) {
 
    audio.addEventListener('click', Tools.delegate('.discovery__audio-play', function (event) {
        const audioArea = event.target.parentNode.querySelector('.discovery__audio-icon');
 
        // Is the clicked audio already playing?
        ...
        const musicFilePath = 'audio/' + musicType + '.mp3';
...
// NEWS: like/dislike a news-article (heart)
if (likeNews) {

    likeNews.addEventListener('click',Tools.delegate ('.news__text-icon', function(event) {
...
// FILTER-VIEW: tiles and list >toggle from one to other
...
function likeIcon(event) {
    ...
    const text = event.target.parentNode.querySelector('.filtered-items__vote-note-text')
    // text.innerText = (text.innerText === 'MERKEN') ? 'GEMERKT' : 'MERKEN'; (SAME SPELLING)
...
if (countdownText) {
    const now = new Date();
    // Date: 18.9., Date() expects months to start with 0
...
// MUSIC-PROGRAM: friday, saturday, sunday
...
programButtons.forEach(function (programBtn) {
    programBtn.addEventListener('click', function (event) {
        ...
        document.querySelector('.lineupFestival__singleDay--active').classList.remove('lineupFestival__singleDay--active');
        // And we hide the currently active program.
        document.querySelector('.lineupFestival__dayProgramm--active').classList.remove('lineupFestival__dayProgramm--active');

        // In this moment we know that no button is active and no program is shown.

        // Then we add the "active" class to the clicked button.
        programBtn.classList.add('lineupFestival__singleDay--active');
        // Get the "data-day" attribute of the clicked button so we know which day was clicked
...
if (video) {
    ...
    video.addEventListener('click',Tools.delegate ('.videoFestival__video-play', function(event) {
...
// ACCORDIONS: more/less text
if (openAccordions) {

    openAccordions.addEventListener('click',Tools.delegate ('.infosFestival__title-moreBtn', function(event) {
        // console.log(event.target.parentNode.parentNode.querySelector('.infosFestival__content'));
        const accord = event.target.parentNode.parentNode.querySelector('.infosFestival__content');

        // Is clicked item already open??
...
window.initMap = function() {
  ...
  const beachMarker = new google.maps.Marker({
    position: { lat: 47.444448, lng: 9.468906 },
    map: map,
    icon: image
  });
}
